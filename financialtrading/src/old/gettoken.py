...
@app.get('/xams/stocks_etfs')
def xams_stocks_and_etfs():
    ...
    for item in all_items:
        result.append({
            'Symbol': item.get('Symbol'),         # e.g. ALFEN:xams
            'Description': item.get('Description'),
            'AssetType': item.get('AssetType'),
            'Uic': item.get('Identifier'),        # summary identifier / instrument id
            'ExchangeId': item.get('ExchangeId'),
            'PrimaryListing': item.get('PrimaryListing'),
        })

    if params['AssetTypes'] == 'Stock':
        with open('stocks.json', 'w') as f:
            json.dump(result, f, indent=4)
    elif params['AssetTypes'] == 'Etf':
...
def oauth_callback():
    ...
    return redirect('/xams/stocks_etfs')
